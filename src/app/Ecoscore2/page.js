...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    const shareToInstagram = async () => {
        const imageUrl = "https://nanyang-james24.vercel.app/image/ecoscore2.png";

        try {
            const response = await fetch(imageUrl);
            const blob = await response.blob();
            const file = new File([blob], "ecoscore2.png", { type: "image/png" });
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center gap-4 z-10">
          ...
          <img
            src={isEnglish ? "/image/ecoscore2EN.png" : "/image/ecoscore2.png"}
            alt="Eco Score Card"
